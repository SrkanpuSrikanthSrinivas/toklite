...
import { reduce, reduceSync } from '../src/reducers.js';
  ...
  switch (cmd) {
    ...
    case 'verify': {
      ...
      for (const item of items) {
        ...
        const { body: reduced } = await reduce(original, 'anthropic', cfg);
    ...
    /* --------------------------------------------------------------- *
     * profile — measure where the tokens actually are, before trying to
     * remove more of them. Ablation counting against the free endpoint,
    ...
    case 'profile': {
      ...
      const items = capture.list().filter(i => i.format === 'anthropic').slice(-limit);
      if (!items.length) {
        console.log('Nothing captured yet:');
        console.log(c.dim('  toklite config set capture.enabled true'));
        console.log(c.dim('  toklite run -- <your agent>'));
        console.log(c.dim('  toklite profile'));
        break;
      }
      ...
      console.log(c.b(`Profiling ${items.length} request(s)`));
      console.log(c.dim('  Each bucket is measured by blanking it out and re-counting.'));
      console.log(c.dim('  Exact, and free — count_tokens is not billed.\n'));

      const results = [];
      for (const item of items) {
        const r = await profile.profileRequest(item.body, 'anthropic', headers, upstream, cfg);
        if (!r.ok) { console.log(c.y(`  skipped: ${r.reason}`)); continue; }
        results.push(r);
        process.stdout.write('.');
      }
      console.log('\n');

      const agg = profile.aggregate(results);
      if (!agg) { console.log(c.y('  Nothing could be measured. Check the API key.')); break; }

      const rows = Object.entries(agg.buckets)
        .map(([name, v]) => ({ name, ...v }))
        .filter(r => r.tokens > 0 || r.bytes > 0)
        .sort((a, b) => b.tokens - a.tokens);

      console.log(c.b(`  ${agg.total.toLocaleString()} input tokens across ${agg.n} request(s)\n`));
      console.log(c.dim('  bucket           tokens     share   already cut'));
      for (const r of rows) {
        const share = agg.total ? ((r.tokens / agg.total) * 100).toFixed(1) + '%' : '—';
        const cut = r.bytes ? ((r.reducedBytes / r.bytes) * 100).toFixed(0) + '% of bytes' : '—';
        console.log(`  ${r.name.padEnd(16)} ${String(r.tokens.toLocaleString()).padStart(8)}  ${share.padStart(7)}   ${cut}`);
      }

      const untracked = agg.total - rows.reduce((a, r) => a + r.tokens, 0);
      if (Math.abs(untracked) > agg.total * 0.02) {
        console.log(c.dim(`  ${'framing'.padEnd(16)} ${String(untracked.toLocaleString()).padStart(8)}   ${'   '}   message envelopes, unremovable`));
      }

      console.log(c.b('\n  Where the next win is'));
      const top = rows.filter(r => r.tokens > 0)
        .map(r => ({ ...r, headroom: r.bytes ? 1 - (r.reducedBytes / r.bytes) : 0 }))
        .sort((a, b) => (b.tokens * b.headroom) - (a.tokens * a.headroom))[0];
      if (top) {
        console.log(`    ${c.b(top.name)} holds ${((top.tokens / agg.total) * 100).toFixed(0)}% of your tokens ` +
                    `with ${(top.headroom * 100).toFixed(0)}% still untouched.`);
        console.log(c.dim(`    ${HINTS[top.name] || 'Inspect a capture to see what is in it.'}`));
      }

      if (agg.breakpoints) {
        console.log(c.y(`\n  ${agg.breakpoints} cache breakpoint(s) seen across the sample.`));
        console.log(c.y('  Tokens behind a breakpoint bill at about a tenth of the input rate,'));
        console.log(c.y('  so removing them is worth roughly a tenth as much as removing fresh'));
        console.log(c.y('  content. Optimise for the bill, not the token count.'));
      }
      break;
    }
...
`;
      ...
      const { body, report } = reduceSync(probe, 'anthropic', loadConfig());
  ...
  toklite profile              where your tokens actually are, measured
...
const HINTS = {
  tool_results: 'File re-reads dominate here. Diff-based re-sends are the next lever: an edited file currently re-sends whole.',
  tools: 'Schemas resent every turn. Trimming unreferenced descriptions is already on; lazy-loading whole tools is the next step.',
  system: 'Usually stable — cache it rather than shrink it. Confirm a breakpoint covers it.',
  user_text: 'Pasted context. Line-range windowing around referenced symbols is the lever.',
  assistant_text: 'Transcript growth. Lower reducers.compact.keepRecentTurns to trim older turns harder.',
  images: 'Anthropic bills roughly (width x height) / 750. Downscaling screenshots before send is the largest single lever here.'
};
